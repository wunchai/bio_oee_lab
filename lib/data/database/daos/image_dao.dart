...
import 'package:bio_oee_lab/data/database/app_database.dart'; // For AppDatabase access
import 'package:bio_oee_lab/data/database/tables/image_table.dart'; // For Images table and DbImage
...
class ImageDao extends DatabaseAccessor<AppDatabase> with _$ImageDaoMixin {
  ...
  Future<DbImage?> getImageByGuid(String guid) {
    return (select(images)..where((tbl) => tbl.guid.equals(guid)))
        .getSingleOrNull();
  }

  /// NEW: Gets the latest lastSync timestamp for images matching a specific syncStatus.
  Future<String?> getLastSyncForImagesBySyncStatus(int syncStatus) async {
    final result = await customSelect(
      'SELECT MAX(lastSync) FROM images WHERE statusSync = ?',
      variables: [
  ...
  Future<int> deleteImagesByDocumentIds(List<String> documentIds) {
    return (delete(images)..where((tbl) => tbl.documentId.isIn(documentIds)))
        .go();
  }

  /// NEW: Deletes images by a list of problemIds.
  Future<int> deleteImagesByProblemIds(List<String> problemIds) {
    return (delete(images)..where((tbl) => tbl.problemId.isIn(problemIds)))
        .go();
  }
  ...
  Future<List<DbImage>> getImagesBySyncStatus(int syncStatus) {
    return (select(images)..where((tbl) => tbl.statusSync.equals(syncStatus)))
        .get();
  }
  ...
  Future<int> countImagesForProblem(String problemId) async {
    ...
    final count =
        await (select(images)..where((tbl) => tbl.problemId.equals(problemId)))
  ...
  Future<DbImage?> getImageByUid(int uid) {
    return (select(images)..where((tbl) => tbl.uid.equals(uid)))
        .getSingleOrNull();
  }
  ...
  Future<int> deleteAllImagesByDocumentId(String documentId) {
    return (delete(images)..where((tbl) => tbl.documentId.equals(documentId)))
        .go();
  }
}
