...
class ProblemDao extends DatabaseAccessor<AppDatabase> with _$ProblemDaoMixin {
  ...
  Future<int> insertProblem(ProblemsCompanion entry) =>
      into(problems).insert(entry);
  ...
  /// NEW: Gets the latest lastSync timestamp from the problems table.
  Future<String?> getLastSync() async {
    final result = await customSelect('SELECT MAX(lastSync) FROM problems')
        .getSingleOrNull();
  ...
  Future<int> deleteProblemsBySyncStatus(int syncStatus) {
    return (delete(problems)..where((tbl) => tbl.syncStatus.equals(syncStatus)))
        .go();
  }
  ...
  Future<DbProblem?> getProblemByUid(int uid) {
    return (select(problems)..where((tbl) => tbl.uid.equals(uid)))
        .getSingleOrNull();
  }

  /// NEW: Gets problem records filtered by syncStatus.
  Future<List<DbProblem>> getProblemsBySyncStatus(int syncStatus) {
    // <<< NEW METHOD
    return (select(problems)..where((tbl) => tbl.syncStatus.equals(syncStatus)))
        .get();
  }
